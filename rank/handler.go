...
func HandleRequestToDetermineRank(rw http.ResponseWriter, req *http.Request) {
	...
	getRank := getRankWithLatLng(convertStringToFloat32(searchTermLat),
		convertStringToFloat32(searchTermLng),
		convertStringToFloat32(realTermLat),
		convertStringToFloat32(realTermLng),
		searchTerm,
		realTerm,
		getDistanceRanker(os.Getenv("OPERATION_TYPE")),
		getLatLngDistanceCalc(os.Getenv("OPERATION_TYPE")),
		getNormalizer(os.Getenv("OPERATION_TYPE")))

	rank := getRank(searchTerm, realTerm, getNormalizer(os.Getenv("OPERATION_TYPE")))
